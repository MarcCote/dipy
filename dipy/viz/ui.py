...
class UI(object):
    ...
    parent_ui: UI
        Reference to the parent UI element. This is useful if there is a parent
...
class ListBox(UI):
    ...
    def __init__(self, values, position, size,
        ...
        self.add_callback(self.panel.panel.actor, up_event, self.up_button_callback)
        self.add_callback(self.panel.panel.actor, down_event, self.down_button_callback)
    ...
    def add_to_renderer(self, ren):
        ...
        super(ListBox, self).add_to_renderer(ren)
    ...
    def _build(self, position):
        ...
        We create as many ListBoxItems objects as it fits given the size.
        ...
        self.panel = Panel2D(center=position, size=self.size, color=(1, 1, 1))

        # Initialisation of empty text actors
        x = 0.1
        Y = np.linspace(0, 1, num=nb_slots, endpoint=False)[::-1]
        for y in Y:
            item = ListBoxItem(list_box=self)
            item.font_size = self.font_size
            item.font_family = "Arial"
            self.slots.append(item)
            self.panel.add_element(item, 'relative', (x, y))

        self.up_button = Button2D({"up": read_viz_icons(fname="arrow-up.png")})
        self.panel.add_element(self.up_button, 'relative', (0.95, 0.95))

        self.down_button = Button2D({"down": read_viz_icons(fname="arrow-down.png")})
        self.panel.add_element(self.down_button, 'relative', (0.95, 0.05))
    ...
    def update(self):
        ...
        for i, choice in enumerate(self.values[view_start:view_end]):
            slot = self.slots[i]
            slot.element = choice
    ...
    def select(self, item, multiselect=False, range_select=False):
        ...
        """
        if not self.multiselection or not (multiselect or range_select):
            self.clear_selection()

        selection_idx = self.values.index(item.element)
        if range_select and selection_idx != self.last_selection_idx:
            step = np.sign(selection_idx - self.last_selection_idx)
            for i in range(self.last_selection_idx, selection_idx + step, step):
                self.selected.add(self.values[i])

        elif multiselect and item.element in self.selected:
            self.selected.remove(item.element)

        else:
            self.selected.add(item.element)
...
class ListBoxItem(TextBlock2D):
    ...
    def __init__(self, list_box):
        ...
        self.actor.GetTextProperty().SetLineSpacing(1)
        self.deselect()
    ...
    def deselect(self):
        ...
        self.actor.GetTextProperty().SetBold(False)
        self.actor.GetTextProperty().SetColor(0, 0, 0)
        self.selected = False

    def select(self):
        # if vtk.vtkVersion.GetVTKSourceVersion().split(' ')[-1] <= "6.2.0":
        #     self.actor.GetTextProperty().SetColor(1, 0.5, 0)
        # else:
        #     self.actor.GetTextProperty().SetBackgroundColor(1, 0.5, 0)
        #     self.actor.GetTextProperty().SetBackgroundOpacity(1.0)

        self.actor.GetTextProperty().SetColor(0, 0, 0)
        self.actor.GetTextProperty().SetBold(True)
        self.selected = True
    ...
    def element(self, element):
        ...
        self.message = "" if self._element is None else str(element)
    ...
    def left_button_clicked(i_ren, obj, list_box_item):
        ...
        obj: :class:`vtkActor`
            The picked actor
        list_box_item: :class:`ListBoxItem`
